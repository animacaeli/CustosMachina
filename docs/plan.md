...
| T3 | 首家实现的 IM | M1 前(选自己团队在用的一家 dogfood) |
...
计划文档作为总纲,按批次细化与实现,避免一次性膨胀:

| 批次 | 范围 | 状态 |
|---|---|---|
| 批次 1 | 骨架 + 权限管理:monorepo 初始化;Go 后端(多数据库 / JWT / casbin RBAC / setup 向导 / 用户与角色 API);前端(登录 / 初始化 / 用户管理 / 角色权限矩阵 / 菜单权限) | 进行中(2026-09-20 启动) |
| 批次 2 | 运行时纳管 + 测试环境槽位(原 M2) | 未开始 |
| 批次 3 | 组件集成层(原 M3) | 未开始 |
| 批次 4 | 告警与 AI 诊断(原 M4) | 未开始 |
| 批次 5 | 开源化打磨(原 M5) | 未开始 |

IM 扫码登录(依赖 T3 首家 IM 选型)安排在批次 2 前后,不阻塞批次 1(本地超管先行)。

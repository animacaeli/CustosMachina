...
单节点是起点,不是最终态。两种运行时形态在项目支持范围内,对应不同的集群规模(**2026-09-23 第三阶段评审:去掉 docker swarm,由 docker-compose 直接过渡到 k3s,且不是全局一刀切切换,而是按项目/环境部署目标共存**——运行时是部署目标上的属性,不同项目可各自选择与迁移,详见 docs/plan-phase3-envs.md):

| 运行时 | 形态 | 交付阶段 |
|---|---|---|
| docker-compose | 单机起步;多机时由人工指定服务分布;实例伸缩用 `--scale` | MVP |
| k3s | 轻量集群:K8s API 兼容,自愈 / 伸缩完备;灰度分流用原生流量能力 | v1.x |

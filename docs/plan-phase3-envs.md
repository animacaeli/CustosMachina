...
> ⑨（2026-09-25 首轮使用反馈）：通知群取消名称前缀约束，用途改为登记时显式选择，发送层按 webhook 地址识别企微/钉钉/飞书并适配各自消息格式；registry 增加 harbor 类型（Harbor 本身不轻量——多容器组件约 2G 内存，仅作为可登记的镜像仓库类型，平台不代部署）；服务器改名主机管理并新增主机配置探测（见架构决策 9）；**项目管理与环境管理的 UI 与用户预期差距大，功能未测，标记为专门迭代项（见"待办"）**。
...
### 9. 主机配置展示（2026-09-25 新增）

- 环境探测顺带采集主机配置并缓存到 `servers.host_info`（JSON）：CPU 型号/核数、内存、系统盘总量与已用、网卡协商速率；列表页"配置"列摘要展示（如 `4C · 8G · 60G(42%) · 1000Mbps`），tooltip 看明细；
- **公网带宽是云厂商账单属性，机器内测不到**——展示的是网卡协商速率，UI 明示；需要真实公网带宽得接云厂商 API（后置）；
- 采集用一条 SSH 命令（nproc / /proc/cpuinfo / free / df / /sys/class/net/*/speed），只在环境探测时刷新（手工触发），不做周期采集（磁盘使用率变化慢，够用）。

### 待办（不排期，用户专门安排）

- ~~项目管理与环境管理 UI 重构~~ ✅ 2026-09-26 已重构（用户给出方案）：取消 /projects 一级目录，功能并入环境管理；/envs 下三个子菜单（正式 / 灰度 / 测试），每个页面都是"项目列表 + 行内操作"——环境操作按子菜单区分（正式：构建/发布；灰度：构建/发布/策略；测试：构建/槽位），管理操作（详情抽屉 = 配置/部署目标/容器、编辑、删除）三页相同，新增项目按钮三页都有。后端 API 不变。

### 7. 镜像仓库与部署描述来源（2026-09-23 定稿）

**镜像仓库（Registry）可配置，支持三种提供方**：阿里云 ACR / 腾讯云 TCR / gitea 内置 registry。

- `registries` 表：名称、类型（aliyun/tencent/gitea/harbor）、地址、加密凭据；管理后台登记（admin），可登记多个；
